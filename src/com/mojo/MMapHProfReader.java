...
public class MMapHProfReader {
  ...
  public MMapHProfReader(MappedByteBuffer out) {
    this.snapshot = new Snapshot(new OriginalMappedReadBuffer(out));
    this.dumpsToSkip = 0;
  ...
  public Snapshot read() throws IOException {
    ...
    System.out.println("Dump file created " + (new Date(out.getLong())));
    currPos += 8;
    ...
    for (; ; ) {
      ...
      out.getInt();       // Timestamp of this record
      ...
      long length = out.getInt() & 0xffffffffL;
      if (debugLevel > 0) {
      ...
      case HPROF_LOAD_CLASS: {
        int serialNo = out.getInt();        // Not used
        long classID = readID();
        int stackTraceSerialNo = out.getInt();
      ...
      case HPROF_TRACE: {
        if (stackTraces == null) {
          ...
          int serialNo = out.getInt();
          int threadSeq = out.getInt();   // Not used
          StackFrame[] frames = new StackFrame[out.getInt()];
  ...
  private void readHeapDump(long bytesLeft, long posAtEnd) throws IOException {
    while (bytesLeft > 0) {
      ...
      case HPROF_GC_ROOT_THREAD_OBJ: {
        long id = readID();
        int threadSeq = out.getInt();
        int stackSeq = out.getInt();
    ...
    if (bytesLeft != 0) {
      warn("Error reading heap dump or heap dump segment:  Byte count is " + bytesLeft
          + " instead of 0");
      skipBytes(bytesLeft);
    }
    if (debugLevel > 0) {
  ...
  private long readID() throws IOException {
    return (identifierSize == 4) ?
        (Snapshot.SMALL_ID_MASK & (long) out.getInt()) : out.getLong();
  }
  ...
  private int readClass() throws IOException {
    long id = readID();
    StackTrace stackTrace = getStackTraceFromSerial(out.getInt());
    long superId = readID();
    ...
    int instanceSize = out.getInt();
    int bytesRead = 7 * identifierSize + 8;

    int numConstPoolEntries = (int) out.getShort();
    bytesRead += 2;
    for (int i = 0; i < numConstPoolEntries; i++) {
      int index = (int) out.getShort(); // unused
      bytesRead += 2;
      bytesRead += readValue(null);       // We ignore the values
    }

    int numStatics = (int) out.getShort();
    bytesRead += 2;
    ...
    int numFields = (int) out.getShort();
    bytesRead += 2;
